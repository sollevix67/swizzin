...
if [[ ${cf} == yes ]]; then
    /root/.acme.sh/acme.sh --force --issue --dns dns_cf -d ${hostname} --test >> $log 2>&1 || {
        echo_error "Certificate could not be issued."
        exit 1
    }
else
    if [[ $main = yes ]]; then
        /root/.acme.sh/acme.sh --force --issue --nginx -d ${hostname} --test >> $log 2>&1 || {
            echo_error "Certificate could not be issued."
            exit 1
        }
    else
        systemctl stop nginx
        /root/.acme.sh/acme.sh --force --issue --standalone -d ${hostname} --pre-hook "systemctl stop nginx" --post-hook "systemctl start nginx" --test >> $log 2>&1 || {
...
/root/.acme.sh/acme.sh --force --install-cert -d ${hostname} --key-file /etc/nginx/ssl/${hostname}/key.pem --cert-file /etc/nginx/ssl/${hostname}/cert.pem --fullchain-file /etc/nginx/ssl/${hostname}/fullchain.pem --ca-file /etc/nginx/ssl/${hostname}/chain.pem --reloadcmd "systemctl reload nginx"
if [[ $main == yes ]]; then
...
if [[ -f /install/.plex.lock ]]; then
    openssl pkcs12 -export -out /var/lib/plexmediaserver/plex_certificate.p12 -in /etc/nginx/ssl/${hostname}/cert.pem -inkey /etc/nginx/ssl/${hostname}/key.pem -certfile /etc/nginx/ssl/${hostname}/chain.pem -passout pass:PASSWORD -certpbe AES-256-CBC -keypbe AES-256-CBC -macalg SHA256 >> $log 2>&1;
